...
![IIOT架构图](./illustrations/iiot.jpg)  
...
`To Input` 选择之前创建的`From Raspberry Agent`
整体如下：
![data_flow_degin.png](illustrations/data_flow_degin.png)

##### MiNiFi 数据流导出
在web界面，按住`shift`, 选择上图右侧的一排流程，右键选择 保存为模板
...
- name: http://king:8080/nifi
  ...
  - id: AUTOGENERATED_NIFI_PORT_ID_HERE
    ...
    Properties: 
        ...
        Host Name: king
```
> 不调整yml会出现 `ERROR [Timer-Driven Process Thread-9] o.a.nifi.remote.StandardRemoteGroupPort RemoteGroupPort[name=From Raspberry Agent,targets=http://king:8080/nifi] failed to communicate with http://king:8080/nifi because the remote instance indicates that the port is not in a valid state`
